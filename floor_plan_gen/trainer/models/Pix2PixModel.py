...
class Pix2Pix(BaseModel):
    ...
    def calculate_D_loss(self, discriminator_b_real, discriminator_b_false):
        ...
        Args:
            discriminator_b_real (tf.tensor): Discriminator prediction for real 
        B image.
            discriminator_b_false (tf.tensor): Discriminator prediction for 
        generated B image.
        ...
        """
        D_true_loss = (
            self.disc_loss_function(self.create_label(True),         
        discriminator_b_real)
            * self.DM_loss_coeff
        )
        D_false_loss = (
            self.disc_loss_function(self.create_label(False),     
    ...
    def batch_train(self, batch, current_step, disc_std):
        ...
        Args:
            ...
            disc_std (tensor): tensor with float value depicting current std for 
            disc noise.
        """
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            discriminator_b_real = self.D(real_target + disc_input_noise,   
            training=True)
        ...
        with self.summary_writer.as_default():
            tf.summary.scalar("model/total_D_loss", total_D_loss, 
            step=current_step)
            tf.summary.scalar("model/total_G_loss", total_G_loss, 
            step=current_step)
            ...
            tf.summary.scalar(
                "model_info/G_lr", self.G_optimizer.learning_rate, 
            step=current_step
            )
            tf.summary.scalar(
                "model_info/D_lr", self.D_optimizer.learning_rate, 
            step=current_step
            )
