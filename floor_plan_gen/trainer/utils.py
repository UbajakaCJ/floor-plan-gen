...
class LrScheduler:
  ...
  def update_learning_rate(self, step, lr, optimizers):
    ...
    """
    curr_lr = self._calculate_lr(step, lr)
...
class PatchGanLabels:
  ...
  def create_labels(self, disc_patch, true_label):
    ...
    if self.label_type == "fixed":
      ...
      lable = tf.random.uniform(minval=low, maxval=high, shape=disc_patch)
      ...
      label = tr.random.uniform(minval=low, maxval=high, shape=disc_patch)
    else:
      label = None
